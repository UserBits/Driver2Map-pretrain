...
class FocalLoss(nn.Module):
    def __init__(self, alpha=[1.0, 3.0, 2.0], gamma=2, reduce='mean'):  # alpha 对应三个类别的权重
        super().__init__()
    ...
    def forward(self, inputs, targets):
        ...
        focal_loss = self.alpha[targets.long()] * (1 - pt) ** self.gamma * BCE_loss
        return focal_loss.mean() if self.reduce == 'mean' else focal_loss.sum()
'''
class FocalLoss(nn.Module):
...
class WeightedBCELoss(nn.Module):
    def __init__(self, pos_weight, class_weights=None):
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        #print(f"bce_loss, class_weights device: {bce_loss.device, self.class_weights.device}")
        #shape: batchsize, C=4, H=200, W=400
        # 类别权重应用
        if self.class_weights is not None:
            weights = self.class_weights.view(1, 4, 1, 1)
            bce_loss = bce_loss * weights
        # 正样本增强
        pos_mask = (target == 1).float()
        loss = self.pos_weight * pos_mask * bce_loss + (1 - pos_mask) * bce_loss
        return loss.mean()
