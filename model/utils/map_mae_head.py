...
#这个是预训练的HD Map Prior Transformer模块以及卷积的BEV输出头
...
class MapVisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self,
        ...
        self.bev_head = BevEncode(inC=kwargs['embed_dim'],   #embed_dim=768,
    ...
    def forward(self, x):
        ...
        x = self.bev_head(x)  #这里用的bev head是没有经过预训练的，因此相关权重无需保存到后续步骤使用
        return x


def vit_base_patch8(**kwargs):  #kwargs:接受任意数量的关键字参数，以字典形式存储
