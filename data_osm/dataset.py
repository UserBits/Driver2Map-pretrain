...
class PMapNetDataset(Dataset):
    def __init__(self, version, dataroot, data_conf, is_train):
        ...
        patch_h = data_conf['ybound'][1] - data_conf['ybound'][0]  #60
        patch_w = data_conf['xbound'][1] - data_conf['xbound'][0]  #30
        canvas_h = int(patch_h / data_conf['ybound'][2])           #400
        canvas_w = int(patch_w / data_conf['xbound'][2])           #200
    ...
    def __getitem__(self, idx):
        ...
        return imgs, trans, rots, intrins, post_trans, post_rots, car_trans, yaw_pitch_roll, vectors
...
class PMapNetSemanticDataset(PMapNetDataset):
    ...
    def __getitem__(self, idx):
        ...
        return imgs, trans, rots, intrins, post_trans, post_rots, car_trans, yaw_pitch_roll, semantic_masks, instance_masks, direction_masks, osm_masks, osm_vectors, masked_map, timestamp, scene_id


def collate_wrapper(batch):
    imgs, trans, rots, intrins, post_trans, post_rots, car_trans, yaw_pitch_rolls, semantic_masks, instance_masks, direction_masks, osm_masks, osm_vectors, masked_maps, timestamps, scene_ids = [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []
    for img, tran, rot, intrin, post_tran, post_rot, car_tran, yaw_pitch_roll, semantic_mask, instance_mask, direction_mask, osm_mask, osm_vector, masked_map, timestamp, scene_id in batch:
        imgs.append(img)
        trans.append(tran)
        rots.append(rot)
    ...
    return imgs, trans, rots, intrins, post_trans, post_rots, car_trans, yaw_pitch_rolls, semantic_masks, instance_masks, direction_masks, osm_masks, osm_vectors, masked_maps, timestamps, scene_ids
...
if __name__ == '__main__':
    ...
    print("start loading v1.0-mini")
    dataset = PMapNetSemanticDataset(version='v1.0-mini', dataroot='./dataset/', data_conf=data_conf, is_train=False)
    print(f"dataset loaded.")
    for idx in range(dataset.__len__()):
        print(f"idx = {idx}")
        imgs, trans, rots, intrins, post_trans, post_rots, car_trans, yaw_pitch_roll, semantic_masks, instance_masks, direction_masks, osm_masks, osm_vectors, masked_map, timestamp, scene_id = dataset.__getitem__(idx)
